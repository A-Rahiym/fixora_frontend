<script lang="ts">
	...
	let { target = '/dashboard' }: Props = $props();
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		submitting = true;
		try {
			const result = await loginWithRedirect(email, password, undefined, sanitizeRedirect(target));
...
>
	<div class="flex w-full max-w-115 flex-col">
		...
		<section class="mt-4 grid grid-cols-2 gap-3">
			...
			>
				<div>
					<span class="block text-xs leading-tight font-semibold text-gray-800">Help Center</span>
			...
			>
				<div>
					<span class="block text-xs leading-tight font-semibold text-gray-800">System Status</span>
